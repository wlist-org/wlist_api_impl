...
public record InternalDownloadInformation(
        long start,
        long len,
        DownloadChunkInformation chunk,
        String optionalUrl,
        String optionalToken,
        byte[] optionalExtra
) implements Serializable, Recyclable {
    public static void serialize(final InternalDownloadInformation self, final MessagePacker packer) throws IOException {
        packer.packLong(self.start);
        packer.packLong(self.len);
        DownloadChunkInformation.serialize(self.chunk, packer);
    ...
    public static InternalDownloadInformation deserialize(final MessageUnpacker unpacker) throws IOException {
        final long start = unpacker.unpackLong();
        final long len = unpacker.unpackLong();
        final DownloadChunkInformation chunk = DownloadChunkInformation.deserialize(unpacker);
        ...
        return new InternalDownloadInformation(start, len, chunk, url, token, extra);
    }
}
